...
import { uploadPresentation, getStorageKey } from "../middleware/upload.middleware.js";
...
router.post("/submit", requireAuth, async (req, res, next) => {
    uploadPresentation(req, res, async (err) => {
        ...
        try {
            ...
            let slideStorageKey = null;
            let slideFilename = null;
            let slideSizeBytes = null;
            if (req.file) {
            ...
            const submission = await submissionService.submit(user.teamId, user.profileId, parsed.repositoryUrl, parsed.deploymentUrl || null, slideStorageKey, slideFilename, slideSizeBytes);
            res.json({
                success: true,
