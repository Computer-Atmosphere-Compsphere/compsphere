...
const fragmentShader = `
  ...
  void main() {
    ...
    // Higher power = fewer, sharper, more sparse sparkles
    result = pow(result, 18.0);

    // iIntensity controls brightness
    gl_FragColor = vec4(vec3(iIntensity) * result, 1.0);
  }
`;
...
interface SparklesPlaneProps {
  speed?: number;
  intensity?: number;
  /** Controls sparkle density — higher = finer/smaller sparkles (default 1.0) */
  uvScale?: number;
}
 ...
 */
function SparklesPlane({ speed = 1, intensity = 2.5, uvScale = 1.0 }: SparklesPlaneProps) {
  ...
  }, [noiseTexture, intensity, uvScale]);
  ...
  return (
    <mesh ref={meshRef} material={material}>
      {/* Wide plane (40 units) ensures full horizontal coverage even on ultra-wide screens */}
...
interface GlitterFinalProps {
  ...
  intensity?: number;
  className?: string;
  /** Controls sparkle size — higher = finer/smaller pattern (default 1.0) */
  uvScale?: number;
  /** Override wrapper inline styles (e.g. for tiled positioning) */
