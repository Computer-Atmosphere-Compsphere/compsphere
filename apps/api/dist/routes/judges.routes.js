...
router.get("/my-assignments", requireAuth, requireRole("JUDGE"), async (req, res, next) => {
    try {
        ...
        const assignmentsWithScores = await Promise.all(assignments.map(async (a) => {
            const score = await db.query.judgeScores.findFirst({
                where: and(eq(schema.judgeScores.judgeId, judge.id), eq(schema.judgeScores.teamId, a.teamId)),
            });
...
router.get("/", requireAuth, requireRole("ADMIN"), async (req, res, next) => {
    try {
        ...
        const judgesWithCounts = await Promise.all(judges.map(async (j) => {
            const [{ count }] = await db
                .select({ count: sql `count(*)::int` })
                .from(schema.judgeAssignments)
                .where(eq(schema.judgeAssignments.judgeId, j.id));
            const [{ scored }] = await db
                .select({ scored: sql `count(*)::int` })
    ...
    try {
        ...
        if (uniqueAssignments.length > 0) {
            ...
            for (let i = 0; i < uniqueAssignments.length; i += 100) {
                ...
                await db.insert(schema.judgeAssignments).values(batch.map((a) => ({
                    judgeId: a.judgeId,
                    teamId: a.teamId,
                })));
            }
...
//# sourceMappingURL=judges.routes.js.map