...
router.get("/teams", async (req, res, next) => {
    try {
        ...
        if (parsed.search) {
            whereClause = or(ilike(schema.competitionTeams.teamName, `%${parsed.search}%`), ilike(schema.competitionTeams.teamCode, `%${parsed.search}%`));
        }
        if (parsed.category) {
...
router.post("/regenerate-team-token", async (req, res, next) => {
    try {
        ...
        res.json({
            success: true,
            message: "New activation token generated. Share securely with the team leader — it will not be shown again.",
            data: result,
        });
    }
    catch (error) {
