...
export function OnboardingAdmin() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<AdminTokenForm>();

  const onSubmit = async (data: AdminTokenForm) => {
    setIsLoading(true);
    setErrorMsg(null);
    try {
      ...
      setErrorMsg(err.message || "Failed to validate admin access token.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <GlassPanel className="space-y-6">
        <div className="space-y-2">
          <h2 className="text-xl font-extrabold text-text-primary flex items-center gap-2">
            ...
            <span>Committee Authenticator</span>
          </h2>
          <p className="text-xs text-text-secondary leading-relaxed">
            Enter your administrative credential token to assign the Admin role and open the Operations Dashboard.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <NeonButton type="submit" disabled={isLoading} className="w-full mt-4">
            {isLoading ? "Authenticating..." : "Assign Admin Role"}
          </NeonButton>
        </form>
      </GlassPanel>
    </div>
