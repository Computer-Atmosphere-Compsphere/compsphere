    ...
    async initiate(adminId) {
        return await db.transaction(async (tx) => {
            ...
            const [config] = await tx.update(schema.battleRoyaleConfig)
                ...
                .returning();
            if (!config)
                throw new AppError(500, "Failed to initiate Battle Royale");
    ...
    async claimSlot(teamId, userId) {
        return await db.transaction(async (tx) => {
            ...
            const team = await tx.query.competitionTeams.findFirst({
                where: and(eq(schema.competitionTeams.id, teamId), eq(schema.competitionTeams.status, "WAITLIST")),
            });
            if (!team) {
...
//# sourceMappingURL=battleRoyale.service.js.map