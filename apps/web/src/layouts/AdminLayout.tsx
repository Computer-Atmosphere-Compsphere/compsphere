...
export function AdminLayout() {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-bg-primary text-text-primary">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-text-muted">Loading Admin session...</p>
        </div>
      </div>
    );
  }
  ...
  const navItems = [
    ...
    { label: "Upload Phase 1", path: "/admin/migration", icon: <Database className="w-4 h-4" /> },
    { label: "All Teams", path: "/admin/teams", icon: <Users className="w-4 h-4" /> },
    { label: "Payments Queue", path: "/admin/payments", icon: <CreditCard className="w-4 h-4" /> },
    { label: "Battle Royale", path: "/admin/battle-royale", icon: <Flame className="w-4 h-4" /> },
    { label: "Submissions", path: "/admin/submissions", icon: <Code2 className="w-4 h-4" /> },
    { label: "QR Attendance", path: "/admin/attendance", icon: <QrCode className="w-4 h-4" /> },
    { label: "Judges Space", path: "/admin/judges", icon: <UserCheck className="w-4 h-4" /> },
    { label: "Audit Logs", path: "/admin/audit", icon: <ShieldAlert className="w-4 h-4" /> },
      ...
      <aside className="w-64 border-r border-border bg-bg-secondary/60 flex flex-col fixed h-full z-45">
        ...
        <div className="p-4 border-b border-border bg-bg-surface/20">
          <div className="flex items-center gap-2.5">
            {user.avatarUrl ? (
              <img src={user.avatarUrl} alt="Avatar" className="w-8 h-8 rounded-full border border-red-900/30" />
            ) : (
              <div className="w-8 h-8 rounded-full bg-red-950/40 border border-red-900/30 flex items-center justify-center text-xs font-bold text-red-400">
                A
              </div>
            )}
            <div className="truncate flex-1">
              <p className="font-bold text-xs text-text-primary truncate">{user.fullName}</p>
              <span className="text-[9px] font-bold text-red-400 bg-red-950/40 border border-red-900/30 px-1.5 py-0.5 rounded mt-1 inline-block uppercase">
                Committee Admin
              </span>
            </div>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center justify-between px-3.5 py-2 rounded-md text-xs font-semibold tracking-wider uppercase transition-all duration-200 ${isActive
                  ? "bg-red-950/20 text-red-400 border border-red-900/20 shadow-[0_0_10px_rgba(239,68,68,0.05)]"
