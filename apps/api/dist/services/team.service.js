...
export const teamService = {
     ...
     */
    async redeemTeamToken(rawToken) {
        ...
        if (!token)
            throw new AppError(400, "Invalid team access token", "INVALID_TOKEN");
        if (token.status === "REVOKED")
            throw new AppError(400, "Token has been revoked", "TOKEN_REVOKED");
        ...
        const existingLeader = await db.query.teamMembers.findFirst({
            where: and(eq(schema.teamMembers.teamId, team.id), eq(schema.teamMembers.role, "TEAM_LEADER"), eq(schema.teamMembers.status, "ACTIVE")),
        });
        return {
            tokenId: token.id,
            team: {
     ...
     */
    async activateAsLeader(profileId, teamId, tokenId) {
        return await db.transaction(async (tx) => {
            // Lock the token row
            const token = await tx.query.teamAccessTokens.findFirst({
                where: and(eq(schema.teamAccessTokens.id, tokenId), eq(schema.teamAccessTokens.teamId, teamId)),
            });
            ...
            const existingLeader = await tx.query.teamMembers.findFirst({
                where: and(eq(schema.teamMembers.teamId, teamId), eq(schema.teamMembers.role, "TEAM_LEADER"), eq(schema.teamMembers.status, "ACTIVE")),
            });
            if (existingLeader) {
     ...
     */
    async joinAsMember(profileId, teamId, tokenId) {
        return await db.transaction(async (tx) => {
            ...
            const existingMember = await tx.query.teamMembers.findFirst({
                where: and(eq(schema.teamMembers.teamId, teamId), eq(schema.teamMembers.userId, profileId), eq(schema.teamMembers.status, "ACTIVE")),
            });
            if (existingMember) {
     ...
     */
    async regenerateTeamToken(adminId, teamId) {
        return await db.transaction(async (tx) => {
            ...
            await tx
                ...
                .set({ status: "REVOKED" })
                .where(and(eq(schema.teamAccessTokens.teamId, teamId), eq(schema.teamAccessTokens.status, "ISSUED")));
            ...
            await tx.insert(schema.notifications).values({
                ...
                message: "Your team has been issued a new activation token. Please contact the committee to receive it.",
            });
            return {
                tokenId: token.id,
                rawToken,
     ...
     */
    async verifyTeam(adminId, teamId) {
        return await db.transaction(async (tx) => {
            const team = await tx.query.competitionTeams.findFirst({
                where: eq(schema.competitionTeams.id, teamId),
            });
     ...
     * Admin: drop a team
     */
    async dropTeam(adminId, teamId, reason) {
        return await db.transaction(async (tx) => {
            const team = await tx.query.competitionTeams.findFirst({
                where: eq(schema.competitionTeams.id, teamId),
            });
    ...
    `);
        ...
        return {
            ...
            payments: {
                ...
                approved: Number(payments?.approved ?? 0),
                rejected: Number(payments?.rejected ?? 0),
            },
            submission_count: Number(submissionMetrics?.submission_count ?? 0),
            attendance_count: Number(attendanceMetrics?.attendance_count ?? 0),
            judging: {
                totalAssignments: Number(judging?.total_assignments ?? 0),
                totalScores: Number(judging?.total_scores ?? 0),
                activeJudges: Number(judging?.active_judges ?? 0),
            },
            recentActivity,
        };
    },
