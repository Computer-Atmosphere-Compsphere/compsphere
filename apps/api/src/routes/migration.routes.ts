...
// Secure all other migration routes
...
router.post("/commit", async (req, res, next) => {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const tokensGenerated: Array<{ teamCode: string; token: string }> = [];
      ...
      for (const row of parsed.rows) {
        ...
        if (row.originalRank <= 30) {
          ...
          await tx
            ...
            .where(
              and(
                eq(schema.teamAccessTokens.teamId, team.id),
  ...
  try {
    ...
    if (lines.length > 0) {
      ...
      for (const line of lines) {
        ...
        if (/^(page|halaman|abstract|abstrak|daftar|kata kunci|kata\s+kunci)/i.test(cleaned)) continue;
        if (/^\d+$/.test(cleaned)) continue;
        // Skip lines that look like institutional headers
        if (/^(universitas|institut|sekolah|fakultas|jurusan|program)/i.test(cleaned)) continue;
...
router.post("/submit-team", upload.single("proposalFile"), async (req, res, next) => {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const maxRankResult = await tx.select({ maxRank: sql<number | null>`max(original_rank)` }).from(schema.competitionTeams);
      ...
      const [team] = await tx
        ...
        .values({
          ...
          teamName: teamName,
          category: uppercaseCategory as "NATIONAL" | "MIX" | "INTERNATIONAL",
